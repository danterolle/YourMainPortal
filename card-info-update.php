<?php
    ...
    $user_id = mysqli_real_escape_string($conn, $_SESSION["user_id"]);
        ...
        $query= "UPDATE `mycard`.`cards` SET `title`='$title', `name`='$name', `surname`='$surname', `email`='$email', `education_experience_id`='$edu_id', `work_experience_id`='$work_id' WHERE `card_id`='$card_id' and `user_id`='$user_id';";
...
$target_dir = "uploads/card_$card_id";
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], "$target_dir.$imageFileType")) {
        ...
        $query = "UPDATE `mycard`.`cards` SET `photo`='$target_dir.$imageFileType' where card_id='$card_id' ;";
        $result = mysqli_query($conn, $query);
if (!$result) {
